...
export async function renderNews(root) {
  ...
  root.innerHTML = `
      ...
      <div id="feeds-list">
        ${feedState.feeds.map((f) => `
          <label class="feed-item">
            ...
            <span><strong>${esc(f.nome)}</strong> — <span class="muted">${esc(f.url)}</span>${f.sync ? ` <span class="muted">(última sync: ${esc(f.sync)})</span>` : ''}</span>
          </label>
        `).join('')}
      </div>
    ...
    <section class="card">
      ...
      <div class="table-wrap">
        <table>
          <thead><tr><th>ID</th><th>Título</th><th>Fonte / Autor</th><th>Categoria</th><th>Data</th><th>Tipo</th><th>Ações</th></tr></thead>
          <tbody>
            ${newsItems.length
              ? newsItems.map((n) => `<tr>
                  <td>${n.id}</td>
                  <td>${n.link
                    ...
                    : esc(n.title)}</td>
                  <td>${esc(n.is_rss ? (n.source || '-') : (n.author_name || '-'))}</td>
                  <td>${esc(n.category || '-')}</td>
                  <td>${new Date(n.created_at).toLocaleDateString('pt-BR')}</td>
                  <td><span style="font-size:.78rem;padding:.15rem .45rem;border-radius:999px;background:${n.is_rss ? '#e8f5e9' : '#e3f2fd'};color:${n.is_rss ? '#0f7a45' : '#1565c0'};">${n.is_rss ? 'RSS' : 'Manual'}</span></td>
                  <td><button data-del="${n.id}" class="secondary" style="width:auto;padding:.3rem .7rem;">Excluir</button></td>
                </tr>`).join('')
              : '<tr><td colspan="7" style="text-align:center;color:#63707c;">Nenhuma notícia cadastrada. Publique manualmente ou sincronize os feeds acima.</td></tr>'}
          </tbody>
        </table>
      </div>
    </section>
  ...
  syncBtn.addEventListener('click', async () => {
    ...
    try {
      ...
      syncStatus.textContent = `✓ ${added} item(s) adicionado(s) ao banco.${warns.length ? ' ⚠ ' + warns.join('; ') : ''}`;
      renderNews(root);
    } catch (err) {
  ...
  root.querySelectorAll('[data-del]').forEach((btn) => {
    btn.addEventListener('click', async () => {
      if (!confirm('Confirmar exclusão desta notícia?')) return;
